...
df <- read.csv("spawning-records.csv")
...
for (rr in 1:length(regionslist)) {
    ...
    pdf(paste0("~/Dropbox/Spawning ProCreator/maps/map", rr, ".pdf"), width=6, height=3)
    par(rep(0, 4))
    ...
    results <- rbind(results, data.frame(country, localities, part.catch, part.value, species=specstr, geocode=paste0("https://www.dropbox.com/home/Spawning%20ProCreator/maps?preview=map", rr, ".pdf"), geotool=url))
...
write.csv(results, "~/Dropbox/Spawning ProCreator/master.csv", row.names=F)
write.csv(allrects, "allrects.csv", row.names=F)
