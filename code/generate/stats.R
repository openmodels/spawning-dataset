...
groups <- read.csv("inputs/saudata/groups.csv")
...
library(PBSmapping)
shp <- importShapefile("inputs/shapefiles/ne_10m_admin_0_countries/ne_10m_admin_0_countries.shp")
...
species <- read.csv("inputs/saudata/species.csv")
groups <- read.csv("inputs/saudata/groups.csv")
groups <- groups[!duplicated(groups),]

countries <- read.csv("inputs/saudata/countries.txt", sep="\t", header=F)
...
write.csv(sumstats2, "outputs/sumstats.csv", row.names=F)
write.csv(species3, "outputs/sau-species.csv", row.names=F)
write.csv(spawning2, "outputs/spawning-species.csv", row.names=F)
